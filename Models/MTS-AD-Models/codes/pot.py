...
def adjust_predicts(score, label, threshold=None, calc_latency=False):
    """
    Calculate predict labels using given `score` and `threshold`.
    Args:
        score (np.ndarray): The anomaly score
        label (np.ndarray): The ground-truth label
        threshold (float): The threshold of anomaly score.
            A point is labeled as "anomaly" if its score exceeds the threshold.
        calc_latency (bool): Not used in this simplified version.
    Returns:
    ...
    # Generate predict labels based only on threshold
